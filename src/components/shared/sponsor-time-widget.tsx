...
function AnalogClock({ date, config }: { date: Date | null; config: SponsorClock }) {
  ...
  return (
    <div className="relative h-28 w-28 shrink-0">
      ...
      <Hand angleDeg={hourAngle} imageUrl={config.hourHandUrl} length={28} thickness={6} color={config.faceColor} />
      <Hand angleDeg={minuteAngle} imageUrl={config.minuteHandUrl} length={42} thickness={4} color={config.faceColor} />
      <Hand angleDeg={secondAngle} imageUrl={config.secondHandUrl} length={48} thickness={2} color="#b5442e" />
...
export function SponsorTimeWidget({ config }: { config: SponsorClock }) {
  ...
  return (
    <div
      className="flex h-full flex-col items-center gap-4 px-5 py-6 text-center text-white"
      style={{ background: config.faceColor }}
    >
      ...
      <div className="flex flex-col items-center gap-1">
        {config.logoUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={config.logoUrl} alt={config.name} className="h-7 w-auto object-contain" />
        ) : (
      ...
      <div className="flex w-full items-center justify-center gap-8 border-t border-white/15 pt-4">
        <div>
